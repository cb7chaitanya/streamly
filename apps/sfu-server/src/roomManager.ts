...
import { AppData, DtlsParameters, Router, RtpCapabilities, Worker } from "mediasoup/node/lib/types.js";
...
export class RoomManager {
    ...
    private async onCreateProducerTransport(ws:WebSocket, peerId: string){
        ...
        try{
            ...
            sendMessage(ws, {
                ...
                payload: {
                    params: transport?.dtlsParameters,
                }
            })
    ...
    private async onConnectProducerTransport(ws:WebSocket, peerId: string, dtlsParameters: DtlsParameters){
    ...
    private async onConnectConsumerTransport(ws:WebSocket, peerId: string, dtlsParameters: DtlsParameters){
    ...
    private onGetProducers(ws:WebSocket, peerId: string){
        ...
        const producersList = producers.filter((producer) => {
    ...
    private async onProduce(ws: WebSocket, peerId: string, payload: any){
        ...
        if(transport){
            ...
            room.informConsumers(producer.id, peerId)
    ...
    private async onConsume(ws: WebSocket, peerId: string, rtpCapabilites: RtpCapabilities){
        const room = this.getRoom(peerId)
        if(!room){
            return
        }
        ...
        const remoteProducerId = producers.filter(({peerId: id}) =>  peerId !== id)[0]?.producer.id
